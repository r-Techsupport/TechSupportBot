    ...
    async def debug_dump(self, ctx):
        ...
        for attachment in ctx.message.attachments:
            if attachment.filename.endswith(".dmp"):
                ...
                if attachment.size == 0:
                    await ctx.send(embed=DumpdbgEmbed(title="Invalid dump detected (Size 0)",
        ...
        try:
            ...
            for dump_url in valid_URLs:
                ...
                response = json.loads(urllib.request.urlopen(req, timeout=100).read().decode('utf-8'))

                # Handling for failed results
                if response["success"] == "false":
                    await ctx.send_deny_embed(f"Something went wrong with debugging! Error: {response['error']}")
                    await self.logger.error(f"Dumpdbg API responded with the error `{response['error']}`")
                    return
            ...
            if len(result_urls) == 1:
                await ctx.send(embed=DumpdbgEmbed(title="Dump succesfully debugged! \nResult links:", description=result_urls))
            else: 
                await ctx.send(embed=DumpdbgEmbed(title="Dumps succesfully debugged! \nResult links:", description=result_urls))
