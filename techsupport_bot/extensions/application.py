...
class ApplicationManager(cogs.LoopCog):
    ...
    async def get_application(
        ...
        Args:
            ...
            allow_old (bool, optional): If this is passed, it will get all applications from the user. Defaults to False.
        """
        if allow_old:
    ...
    async def approve_application(
        ...
        Args:
            ...
            message (str, optional): The message to send to the user. If none is passed, the application is approved silently. Defaults to None.
        """
    ...
    async def deny_application(
        ...
        Args:
            ...
            message (str, optional): The message to send to the user. If none is passed, the application is denied silently. Defaults to None.
        """
        application = await self.search_for_pending_application(member)
        if not application:
            embed = auxiliary.prepare_deny_embed(
    ...
    async def start_application(self, interaction: discord.Interaction) -> None:
        ...
        Args:
            interaction (discord.Interaction): The interaction that requested the application. Can be a button press or slash command
        """
    ...
    async def get_application_from_db_entry(
        ...
        Args:
            ...
            application (bot.models.Applications): The application database entry to get the member from
    ...
    async def build_application_embed(
        ...
        Args:
            ...
            new (bool, Optional): If the application is new and the title should include new. Defaults to True
    ...
    async def get_application_role(
        ...
        Returns:
            Union[discord.Role, None]: Will return the role object from the guild, or none if the role could not be found
        """
    ...
    async def notify_for_application_change(
        ...
        Args:
            message (str): The message provided by the staff. If this is None, no DM will be sent to the user
            approved (bool): Whether this application has been approved. If False, it is denied
            interaction (discord.Interaction): The interaction from the slash command that changed the status
            application (bot.models.Applications): The db entry of the application to update
            member (discord.Member): The member to approve/deny, only for the purposes of sending a DM
        """
