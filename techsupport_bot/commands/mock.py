...
async def generate_mock_message(
    channel: discord.abc.Messageable, user: discord.Member, prefix: str
) -> str:
    ...
    return prepare_mock_message(message.clean_content)


def get_user_to_mock(
    ctx: commands.Context, input_user: discord.Member
) -> discord.Member:
...
def prepare_mock_message(message: str) -> str:
...
class Mocker(cogs.BaseCog):
    ...
    async def mock(
        ...
        Args:
            ...
            input_user (discord.Member): The raw member input by the invoker
        """
        user_to_mock = get_user_to_mock(ctx, input_user)

        prefix = await self.bot.get_prefix(ctx.message)
        mock_message = await generate_mock_message(
