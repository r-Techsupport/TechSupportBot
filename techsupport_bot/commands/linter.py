...
async def check_syntax(message: discord.Message) -> str:
...
def check_valid_attachments(attachments: list[discord.Attachment]) -> bool:
...
class Lint(cogs.BaseCog):
    ...
    async def lint(self: Self, ctx: commands.Context) -> None:
        ...
        """
        if not check_valid_attachments(ctx.message.attachments):
            await auxiliary.send_deny_embed(
                message="You need to attach a single .json file", channel=ctx.channel
            )
            return

        res = await check_syntax(ctx.message)
        if res:
            await auxiliary.send_deny_embed(
